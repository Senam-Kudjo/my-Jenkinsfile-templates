...
function create() {
    ...
    cp template-application.yaml production/applications/"${app_name}".yaml
    cd production/applications/
    ...
    if [ -f "${app_name}.yaml" ]; then
        ...
        sed -i "s#path: production/manifests/app_name#path: production/manifests/${app_name}#" "${app_name}.yaml"
